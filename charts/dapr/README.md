...
| Parameter                                      | Description                                                                                                                                                               | Default |
|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---|
| `dapr_scheduler.ha`                            | If set to true, deploys the Scheduler service with 3 nodes regardless of the value of `global.ha.enabled`                                                                 | `false` |
...
| `dapr_scheduler.cluster.forceInMemoryLog`      | Use in-memory log store and disable volume attach when HA is true                                                                                                         | `false`                 |
| `dapr_scheduler.cluster.logStorePath`          | Mount path for persistent volume for log store in unix-like system when HA is true                                                                                        | `/var/run/dapr/raft-log` |
| `dapr_scheduler.cluster.logStoreWinPath`       | Mount path for persistent volume for log store in windows when HA is true                                                                                                 | `C:\\raft-log`          |
...
| `dapr_scheduler.scaleZero`                     | If true, the StatefulSet is deployed with a zero scale, regardless of the values of `global.ha.enabled` or `dapr_scheduler.ha`                                            | `false` |
